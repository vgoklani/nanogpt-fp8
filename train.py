...
GPU_PEAK_TFLOPS = 438.9e12  # bf16 - RTX6000MaxQ
NUM_GPUS = ddp_world_size  # Number of GPUs being used

grad_accum_steps = max(
    1, math.ceil(total_batch_size / (device_batch_size * ddp_world_size * block_size))
)
total_batch_size = device_batch_size * grad_accum_steps * ddp_world_size * block_size
...
print0(f"{torch.__version__=}")
print0(f"{transformer_engine.__version__=}")
print0(f"{torch.version.cuda=}")
print0(f"{torch.backends.cudnn.version()=}")
# print0(f"{triton.__version__}")

print0("FP8 Block Scaling Support:", te.is_fp8_block_scaling_available())
print0("MXFP8 Support:", te.is_mxfp8_available())
print0("NVFP4 Support:", te.is_nvfp4_available())
print0("Device Compute Capability:", te.get_device_compute_capability())
print0(
...
class LLM(nn.Module):
    def __init__(self):
        ...
        self.blocks = nn.ModuleDict(
            {
                f"block_{i}": te.TransformerLayer(
                    activation="srelu",
    ...
    def forward(self, idx, targets=None, is_first_microbatch=False):
        ...
        if USE_AC_LM_HEAD and self.training and targets is not None:
            loss = te.checkpoint(
                self._lm_head_with_loss, x, targets, use_reentrant=False
            )
...
flops_per_iter = flops_per_token * (device_batch_size * block_size * ddp_world_size)
...
if USE_FSDP2:
    ...
    model = torch.nn.parallel.DistributedDataParallel(
        model, device_ids=[ddp_local_rank], process_group=data_parallel_group
    )
...
nonhidden_params = [
    *raw_model.token_embedding_table.parameters(),
    *raw_model.ln_f.parameters(),
]
...
warmup_scheduler_muon = LinearLR(
    optimizer_muon, start_factor=0.1, total_iters=warmup_iters
)
main_scheduler_muon = CosineAnnealingLR(optimizer_muon, T_max=T_max, eta_min=min_lr)
scheduler_muon = SequentialLR(
    optimizer_muon,
    [warmup_scheduler_muon, main_scheduler_muon],
...
for iter in range(max_iters):
    ...
    if should_print and master_process:
        ...
        tokens_per_iteration = (
            device_batch_size * block_size * iters_since_last_print * ddp_world_size
        )
        ...
        print(
            f"[DEBUG] print+log overhead: {print_overhead:.1f}ms (wandb: {log_overhead:.1f}ms)"
        )
        print("------------")
    ...
    if iter % eval_interval == 0 and iter > 0:
        ...
        if logit_stats is not None:
            print0(
                f"logits: max={logit_stats['max']:.2f}, min={logit_stats['min']:.2f}, mean={logit_stats['mean']:.4f}, std={logit_stats['std']:.4f}"
            )
        print0("------------")
